...
from edge.models.components.layers import PatchEmbed
from edge.models.components.layers.block import Block
from edge.models.components.layers.rope import RotaryPositionEmbedding2D, PositionGetter
from edge.models.components.layers.vision_transformer import vit_small, vit_base, vit_large, vit_giant2
...
class EdgeAggregator(nn.Module):
    ...
    def forward(
        ...
        # DPT heads only read a few intermediate layers; skip unused concats in inference.
        dpt_layers = None
        if not self.training:
            # Default DPT indices used by Edge depth/point heads.
        ...
        for _ in range(self.aa_block_num):
            ...
            for i in range(len(frame_intermediates)):
                if dpt_layers is None or block_i in dpt_layers:
                    # concat frame and global intermediates, [B x S x P x 2C]
                    output_list.append(
                        torch.cat([frame_intermediates[i], global_intermediates[i]], dim=-1)
                    )
                else:
    ...
    def _process_frame_attention(self, tokens, B, S, P, C, frame_idx, pos=None):
        ...
        for _ in range(self.aa_block_size):
            ...
            if self.training:
                tokens = checkpoint(block, tokens, pos, use_reentrant=False)
            else:
            ...
            if kv_cache is not None:
                if self.training:
                    tokens, kv_cache = checkpoint(
                        block, tokens, pos, attn_mask, kv_cache, use_reentrant=False
                    )
                else:
                    tokens, kv_cache = block(tokens, pos, attn_mask, kv_cache)
            else:
                if self.training:
                    tokens = checkpoint(block, tokens, pos, attn_mask, use_reentrant=False)
                else:
                    tokens = block(tokens, pos, attn_mask)
            global_idx += 1
            intermediates.append(tokens.view(B, S, P, C))
