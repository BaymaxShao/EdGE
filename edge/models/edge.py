...
from edge.misc import freeze_all_params
from edge.models.components.aggregator.edge_aggregator import EdgeAggregator
from edge.models.components.heads.camera_head import CameraHead
from edge.models.components.heads.dpt_head import DPTHead


class Edge(nn.Module, PyTorchModelHubMixin):
    def __init__(self, img_size=518, patch_size=14, embed_dim=1024, freeze="none"):
        super().__init__()

        self.patch_size = patch_size
        self.aggregator = EdgeAggregator(img_size=img_size, patch_size=patch_size, embed_dim=embed_dim)
    ...
    def forward(
        ...
        Args:
            ...
            depth_only (bool): If True, skip camera / point heads (faster depth inference).
        ...
        if depth_only:
            # Stay in ambient autocast (bf16/fp16) for speed.
                ...
                if self.camera_head is not None:
                    if camera_head_kv_cache_list is not None:
                        pose_enc_list, camera_head_kv_cache_list = self.camera_head(
                            aggregated_tokens_list, mode=mode, kv_cache_list=camera_head_kv_cache_list
                        )
                    else:
        ...
        if (not depth_only) and camera_head_kv_cache_list is not None:
            predictions["camera_head_kv_cache_list"] = camera_head_kv_cache_list

        if not self.training and not depth_only:
